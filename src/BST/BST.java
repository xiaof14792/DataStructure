...
import Stacks_Queues.Queue.ArrayQueue;
import Stacks_Queues.Queue.Queue;
import Stacks_Queues.Stack.LinkedListStack;
import Stacks_Queues.Stack.Stack;

/**
 * 实现思路： 增--》查（contains）--》遍历（前序遍历、中序遍历、后序遍历、层序遍历、前序遍历非递归实现）--》最小元素、最大元素
 * 删除最小/最大元素--》删除任意元素
 * 
 * @author 14792
 *
...
public class BST<E extends Comparable<E>> {
	...
	private Node add(Node node, E e) {
		...
		if (e.compareTo(node.e) < 0) {// 二分搜索树的一些问题 可以用递归的方法来解决
	...
	public boolean contains(Node node, E e) {
		...
		if (e.compareTo(node.e) == 0) {
			...
			// 正确的代码应该是这样的，最后一个条件是else，没有if，因为只有'>'这最后一个条件了
	...
	// 对二分搜索树进行前序遍历（不采用递归算法），**借助栈结构来模拟系统栈执行逻辑**
	...
	// 返回以root 为根节点的二分搜索树的最小值的节点,也是递归算法实现
	...
	// 后继节点 递归算法
	private Node successor(Node root, E e) {
		if (root == null) {
			return null;
		}

		if (e.compareTo(root.e) >= 0) {
			return successor(root.right, e);
		} else {
			Node right = successor(root.left, e);
			return right == null ? root : right;
		}
	}

	// 前继节点 递归算法
	private Node predecessor(Node root, E e) {
		if (root == null) {
			return null;
		}

		if (e.compareTo(root.e) <= 0) {
			return predecessor(root.left, e);
		} else {
			Node left = predecessor(root.right, e);
	...
	public E floor(E key) {
		if (size == 0 || key.compareTo(minimum()) < 0) {
			return null;
		}
	...
	// 不大于给定参数的最大节点 "<="
	private Node floor(Node node, E key) {
		if (node == null) {
			return null;
		}
	...
	// 返回BST中给定参数的ceil值
	public E ceil(E key) {
		if (size == 0 || key.compareTo(maximum()) > 0) {
			return null;
		}
	...
	// 不小于给定参数的最小节点 ">="
	private Node ceil(Node node, E key) {
		if (node == null) {
			return null;
		}
		...
		Node ceilNode = ceil(node.left, key);
